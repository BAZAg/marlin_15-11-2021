<?php
    $items = GetUsers(); // в это место подать функцию которая берет данные с MySQL
    ...
    function GetUsers(){
        return json_decode(file_get_contents('db_users.json'),true);
    }
    // print_r($items); die;
?>    

<?php function PrintUserData ($person){ ?> 
    <div <?php if($person["is_ban"]) echo 'style="opacity:0.4" '; ?>class="rounded-pill bg-white shadow-sm p-2 border-faded mr-3 d-flex flex-row align-items-center justify-content-center flex-shrink-0">
